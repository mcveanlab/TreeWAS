...
print.help <- function() {
    cat("
          ...
          Example:
          ./grs_tree_analysis.Rscript \
              --sample_file=filename \
              --tree_file=filename \
              --outprefix=text \
              --theta=num --p1=num --num.cores=num \n\n")
...
out <- list(
    treeBF = as.numeric(log10_treeBF),
    t = out.table
)
...
write.table(out$treeBF,file=fout,
            col.names=F,row.names=F,quote=F,sep="\t")

## fout <- paste(outprefix,"_res.rdata",sep='')
## save(out,file=fout)
